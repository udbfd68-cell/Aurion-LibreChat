...
export default defineConfig(({ command }) => ({
  ...
  plugins: [
    ...
    {
      name: 'monorepo-peer-deps-resolver',
      enforce: 'pre' as const,
      resolveId(id, importer) {
        // When resolving bare specifiers from packages/client/dist, fall back to client/node_modules
        if (importer && importer.includes('packages/client/dist') && !id.startsWith('.') && !id.startsWith('/') && !id.startsWith('\0')) {
          // Skip packages listed in resolve.dedupe — let Vite handle them
          // to avoid CJS/ESM dual-package context mismatches
          if (id.startsWith('@ariakit/') || id === 'react' || id === 'react-dom') {
            return null;
          }
          try {
            const resolved = require.resolve(id, { paths: [path.join(__dirname, 'node_modules')] });
            return resolved;
          } catch {
            return null;
          }
      ...
      output: {
        manualChunks(id: string) {
          ...
          if (normalizedId.includes('node_modules')) {
            ...
            // sandpack goes into vendor chunk (needs React in same scope)
            ...
            // codemirror stays in vendor with sandpack (circular deps between them)
  ...
  resolve: {
    dedupe: ['@ariakit/react', '@ariakit/react-core', 'react', 'react-dom', 'i18next', 'react-i18next'],
    alias: {
