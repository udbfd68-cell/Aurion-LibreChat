...
if (!global._agents) global._agents = [
  {
    ...
    description: 'A powerful autonomous AI assistant with 8 real tools: web search, code execution, browser, email, calendar, prospecting, file search, and MCP connectors.',
    instructions: 'You are Aurion Assistant, an autonomous AI agent. You have access to 8 REAL tools — use them proactively. When the user asks for information, ALWAYS search the web first. When asked to run code, ALWAYS execute it. When researching, search from multiple angles. You can chain multiple tool calls. If one approach fails, try another. Format your responses in clear Markdown with headers and links.',
    model: 'nvidia/nemotron-nano-9b-v2:free',
    provider: 'agents',
    model_parameters: {},
    tools: ['web_search', 'execute_code', 'browser', 'calendar', 'prospector', 'send_email', 'read_email', 'file_search'],
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const agent = {
      ...
      model: b.model || 'nvidia/nemotron-nano-9b-v2:free',
