...
module.exports = async function handler(req, res) {
  ...
  if (action === 'direct' && req.method === 'POST') {
    ...
    let thinkTagState = 'detect';
    let contentAccum = '';

    function flushThink(content) {
      if (!content) return;
      reasoningText += content;
      hasReasoning = true;
      sendSSE(res, { event: 'on_reasoning_delta', data: { id: stepId, delta: { content: [{ index: toolContentIndex, type: 'think', think: content }] } } });
    }

    function flushText(content) {
      if (!content) return;
      fullText += content;
      if (hasReasoning && !textStepSent) {
        textStepSent = true;
        sendSSE(res, { event: 'on_run_step', data: { type: 'message_creation', id: textStepId, runId: responseMessageId, index: toolContentIndex + 1, stepDetails: { type: 'message_creation', message_creation: { message_id: responseMessageId } }, usage: null } });
      }
      const sid = hasReasoning ? textStepId : stepId;
      const idx = hasReasoning ? toolContentIndex + 1 : toolContentIndex;
    ...
    function processContent(chunk) {
      contentAccum += chunk;
      if (thinkTagState === 'detect') {
        if (contentAccum.length < 7 && '<think>'.startsWith(contentAccum)) return;
        if (contentAccum.startsWith('<think>')) { thinkTagState = 'think'; hasReasoning = true; contentAccum = contentAccum.substring(7); }
        else { thinkTagState = 'text'; }
      }
      if (thinkTagState === 'think') {
        const endIdx = contentAccum.indexOf('</think>');
        if (endIdx !== -1) { flushThink(contentAccum.substring(0, endIdx)); thinkTagState = 'text'; const after = contentAccum.substring(endIdx + 8); contentAccum = ''; if (after) flushText(after); }
        else if (contentAccum.length > 8) { flushThink(contentAccum.substring(0, contentAccum.length - 8)); contentAccum = contentAccum.substring(contentAccum.length - 8); }
      } else if (thinkTagState === 'text') { flushText(contentAccum); contentAccum = ''; }
    }
    ...
    await new Promise((resolve) => {
      ...
      const apiReq = reqModule.request(reqOpts, (apiRes) => {
        if (apiRes.statusCode !== 200) {
          ...
          apiRes.on('end', () => { streamError = 'Aurion error ' + apiRes.statusCode + ': ' + errBody.substring(0, 500); sendSSEError(res, streamError); res.end(); resolve(); });
          return;
        }
        let buffer = '';
        apiRes.on('data', (chunk) => {
          ...
          for (const line of lines) {
            ...
            try {
              ...
              if (delta?.reasoning) flushThink(delta.reasoning);
              if (delta?.content) processContent(delta.content);
            } catch { /* skip */ }
          }
        });
        apiRes.on('end', () => {
          if (contentAccum) { if (thinkTagState === 'think') flushThink(contentAccum); else flushText(contentAccum); contentAccum = ''; }
          resolve();
        });
      ...
      apiReq.setTimeout(55000, () => { apiReq.destroy(); streamError = 'Aurion timeout'; resolve(); });
    ...
    if (reasoningText) contentParts.push({ type: 'think', think: { value: reasoningText } });
  ...
  if (action === 'stream' && req.method === 'GET') {
    ...
    // State machine for <think> tag parsing in streamed content
    let thinkTagState = 'detect'; // 'detect' | 'think' | 'text'
    let contentAccum = '';        // Accumulator for tag detection
    ...
    function flushText(content) {
      if (!content) return;
      fullText += content;
      // If reasoning was seen, text must go to a DIFFERENT run step (index 1)
      // so the frontend places it in content[1] instead of content[0] where think lives
      if (hasReasoning && !textStepSent) {
        textStepSent = true;
        sendSSE(res, {
          event: 'on_run_step',
    ...
    function processContent(chunk) {
      ...
      if (thinkTagState === 'detect') {
        // Buffer until we can determine if content starts with <think>
        if (contentAccum.length < 7 && '<think>'.startsWith(contentAccum)) {
          return; // Keep buffering
        }
        if (contentAccum.startsWith('<think>')) {
          thinkTagState = 'think';
          hasReasoning = true;
          contentAccum = contentAccum.substring(7);
          // Fall through to process remaining as think
        } else {
          thinkTagState = 'text';
          // Fall through to process as text
        }
      }
      ...
      if (thinkTagState === 'think') {
        const endIdx = contentAccum.indexOf('</think>');
        if (endIdx !== -1) {
          flushThink(contentAccum.substring(0, endIdx));
          thinkTagState = 'text';
          const after = contentAccum.substring(endIdx + 8);
          contentAccum = '';
          if (after) flushText(after);
        } else {
          // Flush all except last 8 chars (might contain partial </think>)
          if (contentAccum.length > 8) {
            flushThink(contentAccum.substring(0, contentAccum.length - 8));
            contentAccum = contentAccum.substring(contentAccum.length - 8);
          }
        }
      } else if (thinkTagState === 'text') {
        flushText(contentAccum);
        contentAccum = '';
      }
    }
    ...
    await new Promise((resolve) => {
      ...
      const apiReq = reqModule2.request(reqOpts2, (apiRes) => {
        ...
        apiRes.on('end', () => {
          ...
          if (contentAccum) {
            if (thinkTagState === 'think') flushThink(contentAccum);
            else flushText(contentAccum);
            contentAccum = '';
          }
          resolve();
        });
