...
/* Projects */
export const getProjects = (includeArchived = false): Promise<q.TProject[]> => {
  const url = includeArchived ? `${endpoints.projects()}?includeArchived=true` : endpoints.projects();
  return request.get(url);
};

export const getProject = (projectId: string): Promise<q.TProject> => {
  return request.get(endpoints.project(projectId));
};

export const createProject = (data: q.CreateProjectPayload): Promise<q.TProject> => {
  return request.post(endpoints.projects(), data);
};

export const updateProject = (
  projectId: string,
  data: q.UpdateProjectPayload,
): Promise<q.TProject> => {
...
/* Skills */
export const getSkills = (category?: string): Promise<q.TSkill[]> => {
  const url = category ? `${endpoints.skills()}?category=${encodeURIComponent(category)}` : endpoints.skills();
  return request.get(url);
};

export const getSkill = (skillId: string): Promise<q.TSkill> => {
  return request.get(endpoints.skill(skillId));
};

export const createSkill = (data: q.CreateSkillPayload): Promise<q.TSkill> => {
  return request.post(endpoints.skills(), data);
};

export const updateSkill = (
  skillId: string,
  data: q.UpdateSkillPayload,
): Promise<q.TSkill> => {
  return request.patch(endpoints.skill(skillId), data);
};

export const deleteSkill = (skillId: string): Promise<{ message: string }> => {
  return request.delete(endpoints.skill(skillId));
};

/* Artifacts (Persistence) */
export const getArtifacts = (conversationId?: string): Promise<q.TArtifact[]> => {
  const url = conversationId
    ? `${endpoints.artifacts()}?conversationId=${encodeURIComponent(conversationId)}`
    : endpoints.artifacts();
  return request.get(url);
};

export const getArtifact = (artifactId: string): Promise<q.TArtifact> => {
  return request.get(endpoints.artifact(artifactId));
};

export const saveArtifact = (data: q.SaveArtifactPayload): Promise<q.TArtifact> => {
  return request.post(endpoints.artifacts(), data);
};

export const shareArtifact = (
  artifactId: string,
  data: q.ShareArtifactPayload,
): Promise<q.TArtifact> => {
  return request.post(endpoints.artifactShare(artifactId), data);
};

export const getSharedArtifact = (shareId: string): Promise<q.TArtifact> => {
  return request.get(endpoints.sharedArtifact(shareId));
};

export const getArtifactVersions = (identifier: string): Promise<q.TArtifact[]> => {
  return request.get(endpoints.artifactVersions(identifier));
};

/* Deep Research */
export const startDeepResearch = (data: q.DeepResearchPayload): Promise<q.DeepResearchResponse> => {
  return request.post(endpoints.deepResearch(), data);
};

/* MCP Registry */
export const getMCPRegistry = (category?: string): Promise<q.TMCPRegistryResponse> => {
  const url = category
    ? `${endpoints.mcpRegistry()}?category=${encodeURIComponent(category)}`
    : endpoints.mcpRegistry();
  return request.get(url);
};
