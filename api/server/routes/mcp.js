...
router.post('/route', async (req, res) => {
  try {
    ...
    // Import MCPRouter dynamically
    const { route } = require('~/server/services/MCP/MCPRouter');
    const servers = route(message);

    return res.status(200).json({ servers });
  } catch (error) {
    logger.error('[MCP Route] Error routing message:', error);
...
/**
 * GET /mcp/registry
 * Returns the pre-configured MCP server registry for 1-click setup
 * Query params: ?category=developer | ?category=featured (returns featured servers)
 */
router.get('/registry', requireJwtAuth, async (req, res) => {
  try {
    const { MCP_REGISTRY, CATEGORIES } = require('~/server/services/MCPRegistry');
    const category = req.query.category;
    let servers = MCP_REGISTRY;
    if (category === 'featured') {
      servers = servers.filter((s) => s.featured === true);
    } else if (category) {
      servers = servers.filter((s) => s.category === category);
    }
    res.json({ servers, categories: CATEGORIES });
  } catch (error) {
    res.status(500).json({ message: error.message || 'Failed to load MCP registry' });
  }
});
